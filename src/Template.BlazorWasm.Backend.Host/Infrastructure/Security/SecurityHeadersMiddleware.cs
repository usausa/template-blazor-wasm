...
public sealed class SecurityHeadersMiddleware
{
    ...
    private Task OnStarting(object state)
    {
        ...
        var policy = $"default-src 'self'; base-uri 'self'; object-src 'none'; form-action 'self'; frame-ancestors 'none'; img-src 'self' data:; font-src 'self'; style-src 'self' 'unsafe-inline'; script-src {scriptSources} 'wasm-unsafe-eval'; connect-src {connectSources}";
        if (reportOnly)
        {
